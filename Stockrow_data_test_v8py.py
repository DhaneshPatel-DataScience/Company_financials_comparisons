...
def processData(ticker_list, row_dict, data_type='metrics', time_type='annual'): 
    df_list=[]
...
def pullData(ticker_list, directory, data_type='metrics', time_type='annual'):
    ...
    for ticker in ticker_list:
        ...
        excel_export_box=WebDriverWait(driver, 100).until(EC.element_to_be_clickable((By.LINK_TEXT, 'Export to Excel (.xlsx)')))
        ...
        print("Data has been pulled for: "+ticker)
    driver.close()
...
def get_row_state():
    ...
    for key, value in checkbutton_dict.items():
        row_boolean.append(value.get())
    final_row_keys=list(compress(list(row_dict.keys()), row_boolean))
    global final_row_dict
    final_row_dict={k:row_dict[k] for k in final_row_keys}
    print("Metrics have been selected...")
    #print(final_row_dict)

def generate_excel_sheet():
    #print(ticker_list)
    #print(final_row_dict)
    processData(ticker_list, time_type='quarterly', row_dict=final_row_dict)
    
#Tkinter GUI implementation
window=tkinter.Tk()
window.title('Company Financial Data Fetching')
label_enter_company=tkinter.Label(window, text= "Please enter company tickers of interest (separated by spaces)")
label_enter_company.grid(row=0, columnspan=3)
#Entry box for typing in company tickers
entry_company=tkinter.Entry(window)
entry_company.grid(row=1, columnspan=3)
choose_metrics_button=tkinter.Button(window, text="Update company selection", command=get_ticker_list)
choose_metrics_button.grid(columnspan=3, row=2)
label_metric_choice=tkinter.Label(window, text="Please enter choose metrics of interest (check all that apply)")
label_metric_choice.grid(row=3, columnspan=3)
grid_row=4
#Following generates checkboxes for rows of interest
for key, value in row_dict.items():
    var=tkinter.BooleanVar()
    check_button=tkinter.Checkbutton(window, text=key, variable=var)
    check_button.grid(columnspan=2, row=grid_row)
    checkbutton_dict[check_button]=var
    grid_row+=1
choose_metrics_button=tkinter.Button(window, text="Update row selection", command=get_row_state)
choose_metrics_button.grid(columnspan=3, row=grid_row)
grid_row+=1
generate_button=tkinter.Button(window, text="Generate excelsheet", command=generate_excel_sheet)
generate_button.grid(columnspan=3, row=grid_row)
grid_row+=1
